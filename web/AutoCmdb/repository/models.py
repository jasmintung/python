...
class AdminInfo(models.Model):
    ...
    用户登录相关信息
    """
    user_info = models.OneToOneField("UserProfile", on_delete=None)
...
class BusinessUnit(models.Model):
    ...
    contact = models.ForeignKey('UserGroup', verbose_name="业务联系人", related_name='c', on_delete=models.CASCADE) # 反向查找时不用XX_set.all(),直接c.all()
...
class Asset(models.Model):
    ...
    latest_date = models.DateField(null=True)  # 最近更新时间
...
class Server(models.Model):
    ...
    服务器信息
    """
    asset = models.OneToOneField('Asset', on_delete=None)

    hostname = models.CharField(max_length=128, unique=True)
    sn = models.CharField('SN号', max_length=64, db_index=True)  # 设置索引
...
class AssetRecord(models.Model):
    ...
    creator = models.ForeignKey('UserProfile', null=True, blank=True, on_delete=None)
    create_at = models.DateTimeField(auto_now_add=True)
