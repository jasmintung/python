(function (jq) {
    ...
    function initialize(pager) {
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            success: function (response) {
                $.Hide('#shade,#loading');  /*去掉遮罩层和加载动态*/
                if (response.status) {
                    console.log(response.data.table_config);
    ...
    function initTableHeader(tbConfig) {
        ...
        $.each(tbConfig, function (k, item) {
            if (item.display) { /*对应idc_service.py文件中table_config中的display字段*/
                var tag = document.createElement('th');
                tag.innerText = item.title; /*对应idc_service.py文件中table_config中的title字段*/
    ...
    function initTbAddEdit(){
        var $header = $('#table_head');
        //console.log($header);
        console.log($header.find('tr th'));
        var $th_list = $header.find('tr th');
        $.each($th_list, function (k, v) {
            if(k > 2){
                /*获取到必须要的列名,创建修改标签*/
                console.log(v.innerText);
                var $lb = document.createElement('label');
                $lb.innerText = v.innerText;
                var $ipt = document.createElement('input');
                $('#modal_add_edit').append($lb);
                $('#modal_add_edit').append($ipt);
            }
        });
    }
    jq.extend({
