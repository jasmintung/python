...
def download(args):
    ...
    :return:head: 文件总大小, content:当前下载数据
    """
    ...
    if os.path.exists(file_path):
        if os.path.isfile(file_path):
            with open(file_path, "rb") as f:
                ...
                protocol_up["data"]["head"] = os.path.getsize(file_path)
...
def accept(sock, mask):
    ...
    conn.settimeout(0.2)  # 这个超时一定要加上,否则会报错:BlockingIOError: [WinError 10035] 无法立即完成一个非阻止性套接字操作
    sel.register(conn, selectors.EVENT_READ, read)


def read(conn, mask):

    try:  # 防止客户端异常关闭导致服务端socket异常崩溃
...
def main():
    ...
    sock.settimeout(0.5)  # 这个超时一定要加上,否则会报错:BlockingIOError: [WinError 10035] 无法立即完成一个非阻止性套接字操作
